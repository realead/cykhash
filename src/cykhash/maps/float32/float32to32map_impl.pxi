...
cpdef Float32to32Map Float32to32Map_from_int32_buffer(key_float32_t[:] keys, int32_t[:] vals, double size_hint=0.0):
...
cpdef Float32to32Map Float32to32Map_from_float32_buffer(key_float32_t[:] keys, float32_t[:] vals,double size_hint=0.0):
    cdef Py_ssize_t n = len(keys)
    cdef Py_ssize_t b = len(vals)
    if b < n:
        n = b
...
cpdef size_t Float32to32Map_to_int32(Float32to32Map map, key_float32_t[:] keys, int32_t[:] vals, bint stop_at_unknown=True, int32_t default_value=0) except *:
    ...
    cdef size_t res = 0
    for i in range(n):
        k = kh_get_float32to32map(map.table, keys[i])
...
cpdef size_t Float32to32Map_to_float32(Float32to32Map map, key_float32_t[:] keys, float32_t[:] vals, bint stop_at_unknown=True, float32_t default_value=float("nan")) except *:
    """returns number of found keys"""
    if map is None:
        raise TypeError("'NoneType' is not a map")
    if map.for_int:
