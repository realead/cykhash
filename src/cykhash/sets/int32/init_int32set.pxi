...
cdef extern from *:
    ...
    /*
        ...
        static inline __attribute__ ((__unused__)) kh_int32set_t *kh_init_int32set(void) { 
            return (kh_int32set_t*)calloc(1,sizeof(kh_int32set_t)); 
        } 


        static inline __attribute__ ((__unused__)) void kh_destroy_int32set(kh_int32set_t *h) { 
            if (h) { 
                free((void *)h->keys); 
                free(h->flags); 
                free((void *)h->vals); 
        ...
        static inline __attribute__ ((__unused__)) void kh_clear_int32set(kh_int32set_t *h) { 
            if (h && h->flags) { 
        ...
        static inline __attribute__ ((__unused__)) khint_t kh_get_int32set(const kh_int32set_t *h, khint32_t key) { 
            if (h->n_buckets) { 
        ...
        static inline __attribute__ ((__unused__)) int kh_resize_int32set(kh_int32set_t *h, khint_t new_n_buckets) { 
            ...
            { 
                ...
                if (h->size >= (khint_t)
                    ...
                    if (h->n_buckets < new_n_buckets) { 
                        khint32_t *new_keys = (khint32_t*)realloc((void *)h->keys,new_n_buckets * sizeof(khint32_t)); 
                        if (!new_keys) { 
                            free(new_flags); 
                            return -1; 
                        } 
            ...
            if (j) { 
                ...
                if (h->n_buckets > new_n_buckets) { 
                    h->keys = (khint32_t*)realloc((void *)h->keys,new_n_buckets * sizeof(khint32_t)); 
                    if (0) 
        ...
        static inline __attribute__ ((__unused__)) khint_t kh_put_int32set(kh_int32set_t *h, khint32_t key, int *ret) { 
            khint_t x; 
            if (h->n_occupied >= h->upper_bound) { 
                if (h->n_buckets > (h->size<<1)) { 
                    if (kh_resize_int32set(h, h->n_buckets - 1) < 0) { 
                        *ret = -1; 
                        return h->n_buckets; 
                    } 
                } 
                else if (kh_resize_int32set(h, h->n_buckets + 1) < 0) { 
                    *ret = -1; 
                    return h->n_buckets; 
                } 
            } 
        ...
        static inline __attribute__ ((__unused__)) void kh_del_int32set(kh_int32set_t *h, khint_t x) { 
